...
class TestStatistikAusnahmen {
	...
	void testLeereStatistiken() {
		...
		String tests[] = { "Leere Statistik soll nicht in String gewandelt werden",
				"Mittelwert einer leeren Statistik soll nicht ausgegeben werden", "Keine Ausnahme erwartet ...", };
		int numTest = 0;
		...
		for (String test : tests) {
			...
			try {
				...
				if (e instanceof NullPointerException) {
					System.out.println("Empty statistic cannot be serialized.");
				} else if (e instanceof ArithmeticException) {
					System.out.println("No average for an empty statistic available.");
				}
			}
		}
	}
	...
	void testNames() {
		...
		for (String[] test : tests) {
			...
			} catch (RuntimeException e) { // Hier sollte eine Ausnahme für die ersten drei Namen gefangen werden
				if(e instanceof NoSuchElementException) {
					System.out.println("Name of statistic must not be empty.");
				}else if (e instanceof NullPointerException) {
					System.out.println("Name of statistic must not be null");
				} //else if(e instanceof  NoSuchElementException) {
				//	System.out.println("Name of statistic must start with a letter.");
				//}
			}
		}
	}
